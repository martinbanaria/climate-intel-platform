    ...
    try:
        ...
        if sort == "best":
            items.sort(key=lambda x: (
                0 if x.get("status") == "MURA" else 1 if x.get("status") == "STABLE" else 2,
                -x.get("savings", 0)
            ))
        elif sort == "price-low":
...
async def get_climate_correlations():
    ...
    try:
        ...
        correlations = analytics_engine.correlate_climate_to_prices(market_items, climate_metrics)
...
async def run_comprehensive_integration(days: int = 7):
    ...
    """
    try:
        logger.info(f"Starting comprehensive real data integration (last {days} days)...")
        ...
        if success:
            ...
            sample = await db.market_items.find_one({}, {'metadata': 1, 'name': 1, 'trend': 1})
            ...
            return JSONResponse({
                ...
                "sample": {
                    ...
                    "date_range": sample.get('metadata', {}).get('date_range') if sample else None
                }
            })
        else:
            return JSONResponse({
...
async def get_integration_status():
    ...
    try:
        ...
        latest = await db.market_items.find_one(
            {},
            sort=[("lastUpdated", -1)]
        )
        ...
        if latest:
            return JSONResponse({
                "success": True,
                "last_update": latest.get("lastUpdated").isoformat() if latest.get("lastUpdated") else None,
...
async def get_grid_status():
    ...
    try:
        ...
        grid_data = {
            ...
            'grids': [
                {'name': 'Luzon', 'capacity': 9845, 'current': 8567, 'status': 'OPTIMAL'},
                {'name': 'Visayas', 'capacity': 3234, 'current': 2987, 'status': 'NORMAL'},
                {'name': 'Mindanao', 'capacity': 3021, 'current': 2680, 'status': 'NORMAL'}
            ]
        }
        return JSONResponse({
            "success": True,
...
async def get_energy_analytics():
    ...
    try:
        ...
        under_construction = [p for p in ppa_list if p.get('status') == 'Under Construction']
        ...
        analytics = {
            'ppa_summary': {
                ...
                'under_construction_capacity': sum(p.get('capacity_mw', 0) for p in under_construction),
