...
class DailyPriceIndexParser:
    ...
    def _construct_daily_url(self, date: datetime) -> str:
        """Construct Daily Price Index PDF URL"""
        month_name = date.strftime("%B")
        return f"{self.base_url}/{date.year}/{date.month:02d}/Daily-Price-Index-{month_name}-{date.day}-{date.year}.pdf"
    
    async def download_pdf(self, date: datetime) -> Optional[bytes]:
        """Download Daily Price Index PDF for a specific date"""
        url = self._construct_daily_url(date)
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, headers=self.headers, timeout=30) as response:
                    if response.status == 200:
                        logger.info(f"Successfully downloaded PDF for {date.strftime('%Y-%m-%d')}")
    ...
    async def download_multiple_days(self, days: int = 7) -> List[Tuple[datetime, bytes]]:
    ...
    def parse_prices(self, text: str) -> Dict[str, float]:
        ...
        for line in lines:
            ...
            if line.isupper() and len(line.split()) <= 4 and not any(char.isdigit() for char in line):
            ...
            match = re.match(r'^([A-Za-z\s]+)\s+([A-Za-z\s\(\)\-,/0-9]+?)\s+(\d+\.?\d*)$', line)
            if match:
    ...
    def _clean_commodity_name(self, name: str) -> Optional[str]:
        ...
        skip_keywords = ['page', 'commodity', 'specification', 'prevailing', 'retail', 'price', 
                        'department', 'agriculture', 'national', 'capital', 'region', 'market',
                        'table', 'source', 'average', 'date', 'prepared']
        ...
        name = re.sub(r'\s+(local|imported|fresh|frozen|dried|sliced|whole|medium|large|small)\s*$', '', name, flags=re.IGNORECASE)
